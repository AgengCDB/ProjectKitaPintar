<?php
  include 'header.php';
  $subjectquery = mysqli_query($conn, "SELECT subjectname FROM subject WHERE subjectid='".$_GET['subjectID']."'");
?>
  ...
  <div class="row">
    <main class="col-md-12 ms-sm-auto px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <?php
        while($hasilsub = mysqli_fetch_array($subjectquery)) { ?>
        <h1 class="h2 mx-5"><?php echo $hasilsub['subjectname']?></h1>
        <?php } ?>
      </div>
      <div>
        <table class="table">
...
-->
          <tbody>
            <?php
                $select = mysqli_query($conn, "SELECT questionid, u.userid, u.username, s.subjectid, s.subjectname, questiontext, questionup, questiondown FROM question q INNER JOIN users u ON q.userid = u.userid INNER JOIN subject s ON s.subjectid = q.subjectid WHERE q.subjectID='".$_GET['subjectID']."'");
